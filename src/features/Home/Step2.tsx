...
import { updateTransaction, deleteMultipleTransactions, updateMultipleTransactions, WebhookBatchResponse } from '../../services/supabaseService';
...
const Step2 = ({ batchResult, isProcessing, banks }: Step2Props) => {
  ...
  React.useEffect(() => {
    if (batchResult && batchResult.status === 'success' && Array.isArray(batchResult.data)) {
  ...
  const handleCellUpdate = async (transactionId: string, columnName: string, newValue: string | number) => {
    try {
